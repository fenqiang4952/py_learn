...
for i in range(0,10):
    ...
    tomato.to_excel('./dist/西虹市首富.xlsx',index=False)                           
    # except:
        # continue
       
## 可以直接读取我们已经爬到的数据进行分析
tomato_com = pd.read_excel('./dist/西虹市首富.xlsx')
...
# data=[(city_com['city'][i] + '市',city_com['count'][i]) for i in range(0,city_com.shape[0])]
...
overlap.render('./dist/主要城市评论数_平均分.html')
...
back_color = imread('./resource/西红柿.jpg')  # 解析该图片
wc = WordCloud(background_color='white',  # 背景颜色
               max_words=200,  # 最大词数
               mask=back_color,  # 以该参数值作图绘制词云，这个参数不为空时，width和height会被忽略
               max_font_size=300,  # 显示字体的最大值
               font_path="./resource/LongShuHongYiTiXiJian-2.otf",  # 解决显示口字型乱码问题，可进入C:/Windows/Fonts/目录更换字体
